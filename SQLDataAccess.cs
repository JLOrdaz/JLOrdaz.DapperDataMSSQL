...
{
    public async Task<IEnumerable<T>> LoadData<T, U>(string storeProcedure, U parameters, string connectionId = "DB")
    {
    ...
    public async Task<T?> LoadFirst<T, U>(string storeProcedure, U parameters, string connectionId = "DB")
    {
        using IDbConnection conex = new SqlConnection(configuration.GetConnectionString(connectionId));
        return await conex.QueryFirstOrDefaultAsync<T>(storeProcedure, parameters, commandType: CommandType.StoredProcedure);
    }

    public async Task SaveData<T>(string storeProcedure, T parameters, string connectionId = "DB")
    {
        using IDbConnection conex = new SqlConnection(configuration.GetConnectionString(connectionId));
